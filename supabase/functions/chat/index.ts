...
async function runAI(ctx: any) {
  ...
  const system = [
    ...
    `CONFIDENCE (CRITICAL — be brutally honest, scores 0.0–1.0): set FOUR scores separately, because you can be sure of one thing and unsure of another:
- "intent_confidence": how sure you are WHAT the customer wants (book vs cancel vs reschedule vs just asking).
- "service_confidence": how sure you are WHICH service they mean.
- "time_confidence": how sure you are of the EXACT day AND time.
- "risk_score": 0.0–1.0 how risky/ambiguous this message is (conflicting info, vague words like "maybe/around/sometime", multiple options) — higher = riskier.
Also set "confidence" = the overall (the minimum of the three). Use LOW scores (below 0.7) whenever you are guessing. The system will NOT execute book/cancel/reschedule when the relevant score is low or risk is high — it will ask a TARGETED clarifying question instead. This protects against costly mistakes.`,
  ...
  // CONFIDENCE GATING (i granuluar): AI s'kontrollon kurrë veprime kritike kur s'është i sigurt.
  // Nota të ndara (intent/service/time + risk) → sqarim i SYNUAR. Konfirmimi i klientit s'bllokohet kurrë.
  const CONF_MIN = 0.7, RISK_MAX = 0.5;
  const num = (v: any, d: number) => (typeof v === "number" ? v : d);
  const baseC = num(out.confidence, 1);
  const intentC = num(out.intent_confidence, baseC);
  const serviceC = num(out.service_confidence, baseC);
  const timeC = num(out.time_confidence, baseC);
  const risk = num(out.risk_score, 0);
  const overallConf = Math.min(intentC, serviceC, timeC);
  const aiSq = isSqLang(biz, out.lang);
  const confirmingNow = !!(ctx.state && ctx.state.step === "awaiting_confirm");
  // Kthen mesazh sqarues të synuar nëse veprimi s'duhet ekzekutuar; null = OK të veprosh.
  function gateBooking(): string | null {
    if (confirmingNow) return null;
    if (risk > RISK_MAX || intentC < CONF_MIN) return aiSq ? "Që të mos gabojmë — ma konfirmo: çfarë shërbimi, cilën ditë dhe orë dëshiron? 😊" : "Just to be sure — could you confirm: which service, day and time would you like? 😊";
    if (serviceC < CONF_MIN) return aiSq ? "Për të qenë i saktë — për cilin shërbim e dëshiron rezervimin? 😊" : "To be precise — which service would you like to book? 😊";
    if (timeC < CONF_MIN) return aiSq ? "Që ta vendos saktë orarin — për cilën ditë dhe orë saktësisht? 😊" : "To get the time exactly right — which day and time precisely? 😊";
    return null;
  }
  const gateDestructive = (): boolean => !confirmingNow && (risk > RISK_MAX || intentC < CONF_MIN);
  ...
  if (out.wants_to_book && out.service && out.date && out.time) {
    ...
    if (svc) {
      ...
      const blk = gateBooking();
      if (blk) {
        // Konfidencë e ulët (në dimensionin përkatës) → mos propozo; kërko sqarim të synuar
        reply = blk;
      } else if (confirming) {
  ...
  if (out.wants_to_reschedule && out.date && out.time && !booked && !cancelled) {
    if (gateDestructive() || (!confirmingNow && timeC < CONF_MIN)) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // GAP #2 — Observability: log çdo ndërveprim (jo në preview) për metrika
    // (booking rate, escalation rate, low-confidence rate, fallback rate...). Best-effort.
    if (!isPreview && result && typeof result === "object") {
      try {
        await supabase.from("ai_events").insert({
          business_id, channel: channel || null, chat_id: chat_id || null,
          via: result.via || null,
          intent: result.intent || (result.booked || result.proposed ? "booking" : result.cancelled ? "cancel" : null),
          confidence: typeof result.confidence === "number" ? result.confidence : null,
          booked: !!result.booked, proposed: !!result.proposed, cancelled: !!result.cancelled,
          escalated: !!result.escalated,
        });
      } catch (_e) { /* tabela ai_events mund të mungojë para SQL → injoro */ }
    }
